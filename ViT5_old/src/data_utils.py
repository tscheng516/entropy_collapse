"""
Data utilities for the ViT-5 entropy-collapse experiments.

Identical to ViT/src/data_utils.py — the data backend is shared between
the ViT/ and ViT5/ experiments.  CIFAR-100 and ImageNet-1k are downloaded
and cached to ``data_dir`` (default ``./data`` relative to the working
directory), which is the same location used by the ViT/ experiment when
...
Supports the following dataset backends:
  ...
  * ImageNet-1k — loaded from a local ImageFolder directory tree if
                  present, otherwise downloaded and cached via Hugging
                  Face ``datasets``.

All datasets are returned as ``(train_loader, val_loader)`` pairs.
Images are resized to ``img_size × img_size`` and normalised with
ImageNet statistics.
"""
...
def load_data(
    ...
    ``DistributedSampler`` so each rank receives a disjoint shard.
    ...
    Returns:
        train_loader, val_loader, train_sampler (None outside DDP).
    """
...
def infinite_loader(loader: DataLoader, sampler: DistributedSampler | None = None):
    ...
    Args:
        ...
        sampler: Optional ``DistributedSampler`` for re-shuffling per epoch.
