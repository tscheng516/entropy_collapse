...
def get_curvature_metrics(
    ...
    try:
        ...
        for _ in range(max_iter):
            ...
            flat_fv = torch.cat([g.contiguous().reshape(-1) for g in vjp_f]) / B
    ...
    # ---- 7. BFGS central-difference (compute_fd only) ----
    bfgs_norm = 0.0
    if compute_fd:
        try:
            eps_cd = 1e-4
    ...
    if compute_fd:
        try:
            ...
            for name in a_cache:
                ...
                max_kfac = max(max_kfac, _top_eig(A_cov) * _top_eig(G_cov))
            kfac_norm = max_kfac
            del a_cache, g_cache
        except Exception:
            pass
    ...
    # ---- 9. FD forward-difference (compute_fd only) ----
    fd_norm = 0.0
    if compute_fd:
        try:
            eps_fd = 1e-4
...
def get_attention_entropy(model: torch.nn.Module) -> list[float]:
    ...
    each transformer layer, using the ``last_att`` tensors cached by the
    patched attention forward pass.
    ...
    Returns:
        List of length equal to the number of transformer blocks, each
    ...
    for block in blocks:
        ...
        if att is not None:
            entropy = -torch.sum(att * torch.log(att + 1e-9), dim=-1)  # (B, heads, N)
