...
Mirrors ``ViT/plot_history.py`` with depth-specific metrics:
  ...
  * Curvature and spike co-occurrence plots are unchanged.
...
Override frequencies (default 500)::
    ...
    python plot_history.py out/nyudepth_vitb16/.../history.pkl \\
...
def _write_analysis_md(
    ...
    lines = [
        ...
        f"- **Source**: `{pkl_path}`",
        f"- **Smoothing λ**: {lam}",
        f"- **Hessian intv**: {hessian_intv}",
        f"- **Entropy intv**: {entropy_intv}",
        "",
    ]
    ...
    if train_config:
        ...
        _cfg_groups = [
            ("Model", ["model_name", "img_size", "patch_size", "depth",
                        "num_heads", "embed_dim",
                        "init_std", "use_scaled_init", "qk_norm"]),
            ("Data", ["data_dir", "batch_size", "num_workers"]),
            ("Loss", ["silog_lambda"]),
            ("Optimiser", ["optimizer", "learning_rate", "max_iters",
                           "weight_decay", "beta1", "beta2", "grad_clip", "eps"]),
        ...
        extra_rows = [[k, str(v)] for k, v in sorted(train_config.items())
                      if k not in covered]
        if extra_rows:
            lines.append("### Other")
            lines.append("")
    ...
    for z, spike_results in sorted(spike_all.items()):
        ...
        if rows:
            lines.append(
                _md_table(["Pair", "n_H_spikes", "n_joint", "P(Y|X spike)"], rows)
            )
        lines.append("")
...
def plot_history(
    ...
    """
    Load a ``history.pkl`` from ``depth/base_train.py`` and reproduce all
    post-training plots and analysis.
    ...
    Outputs:
        ...
        * ``analysis.md``  — structured Markdown report

    Args:
        pkl_path:     Path to a ``history.pkl`` file.
        out_dir:      Output directory; defaults to same dir as pkl_path.
        hessian_intv: Hessian interval used during training.
        entropy_intv: Entropy interval used during training.
        skip_intv:    True (default) = interval-skipping.
...
def main():
    parser = argparse.ArgumentParser(
        description="Re-run post-training plots and analysis from a depth history.pkl."
    )
