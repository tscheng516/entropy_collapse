...
Functions
---------
smooth_log_trend         — Whittaker–Henderson log-space smoother.
...
# ======================================================================
# Smooth log-trend extraction  (Whittaker–Henderson)
# ======================================================================
...
def smooth_log_trend(
    ...
    (higher → smoother).  lam=10 works well for typical curvature traces.
...
def get_curvature_metrics(
    ...
    Always-active proxies (six):
      ...
      * ``diag_h``     — max(diag(H)) via Hutchinson diagonal estimator.
    ...
    Shared computation graph:
      Proxies 1–4 (hessian, hessian_vv, prec_h, diag_h) share a single
      ``create_graph=True`` forward pass.  The graph is released together
      after diag_h completes, avoiding redundant forward passes.

    Args:
        model:              Model, unwrapped from DDP.
        optimizer:          Current optimiser (reads Adam's second moment).
        X:                  Training-batch token ids  (B, T).
        Y:                  Training-batch targets    (B, T).
        vv_mask:            Output of ``get_VV_subspace_mask(model)``.
        max_iter:           Power-iteration steps for λ_max estimation.
        compute_fd:         Enable optional proxies: bfgs, fd, kfac.
        hessian_batch_size: Slice X/Y to this many sequences before any
                            computation, to bound peak GPU memory.
    ...
    """
    # Diagnostic mini-batch — bound peak activation memory
    ...
    # Flash / fused attention is disabled so second-order autograd can
    ...
    # Shared first-order gradient graph (retain for H, H_VV, prec_H, diag_H)
    ...
    #
    # Runs before releasing flat_grads so it shares the computation graph
    # with proxies 1–3.
    # ------------------------------------------------------------------ #
    diag_h_norm = 0.0
    ...
    # Calls model(Xc, Yc) to get full-sequence logits (B, T, V).
    # ------------------------------------------------------------------ #
    def _fwd_tuple(*p_vals: torch.Tensor) -> torch.Tensor:
        p_dict = {k: v for k, v in zip(params_keys, p_vals)}
        return functional_call(model, p_dict, (Xc, Yc))[0]  # (B, T, V)
    ...
    for _ in range(max_iter):
        ...
        l = F.cross_entropy(out_logits.reshape(-1, vocab_size), Yc.reshape(-1))
    ...
    # ------------------------------------------------------------------ #
    # 6. Empirical Fisher  λ_max(F)  via JVP/VJP power iteration
    #
    # F = (1/N) Σ_i g_i g_i^T   where N = B*T token positions.
    # Fisher-vector product: Fv = (1/N) J_ℓ^T (J_ℓ v).
    # ------------------------------------------------------------------ #
    fisher_norm = 0.0
    try:
        def _fwd_loss_tuple(*p_vals: torch.Tensor) -> torch.Tensor:
            """Per-token cross-entropy losses (B*T,)."""
            p_dict = {k: v for k, v in zip(params_keys, p_vals)}
            logits = functional_call(model, p_dict, (Xc, Yc))[0]  # (B, T, V)
            vocab = logits.size(-1)
            return F.cross_entropy(
                logits.reshape(-1, vocab), Yc.reshape(-1), reduction="none"
            )  # (B*T,)

        BT = Xc.size(0) * Xc.size(1)
    ...
    if compute_fd:
        try:
            ...
            for _ in range(max_iter):
                # Split v_b into per-parameter tangents
                offset = 0
                tangents_b: list[torch.Tensor] = []
                for p in params_vals:
                    numel = p.numel()
                    tangents_b.append(v_b[offset : offset + numel].view_as(p))
                    offset += numel

                # Perturb +eps
                for p, t in zip(params_vals, tangents_b):
                    p.data.add_(eps_cd * t)
                optimizer.zero_grad()
                _, loss_bp = model(Xc, Yc)
                loss_bp.backward()
                g_plus = torch.cat(
                    [p.grad.reshape(-1).clone() for p in model.parameters()]
                )

                # Perturb -2*eps (now at -eps from original)
                for p, t in zip(params_vals, tangents_b):
                    p.data.add_(-2.0 * eps_cd * t)
                optimizer.zero_grad()
                _, loss_bm = model(Xc, Yc)
                loss_bm.backward()
                g_minus = torch.cat(
                    [p.grad.reshape(-1).clone() for p in model.parameters()]
                )
    ...
    if compute_fd:
        try:
            ...
            for _ in range(max_iter):
                offset = 0
                tangents_fd: list[torch.Tensor] = []
                for p in params_vals:
                    numel = p.numel()
                    tangents_fd.append(v_fd[offset : offset + numel].view_as(p))
                    offset += numel

                for p, t in zip(params_vals, tangents_fd):
                    p.data.add_(eps_fd * t)

                optimizer.zero_grad()
                _, loss_fp = model(Xc, Yc)
                loss_fp.backward()
                g_pert = torch.cat(
                    [p.grad.reshape(-1).clone() for p in model.parameters()]
                )
