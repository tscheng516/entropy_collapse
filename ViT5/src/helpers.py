...
def smooth_log_trend(
    ...
    Args:
        ...
        lam:     Smoothing strength (λ).
        eps:     Floor added before logging.
        use_abs: If True, take |y| before logging.

    Returns:
        trend_raw, trend_log, log_y
    """
...
def get_VV_subspace_mask(model: torch.nn.Module) -> torch.Tensor:
    ...
    ViT-5 (like timm ViT) fuses Q, K, V into a single ``attn.qkv`` linear
    layer of shape ``(3*dim, dim)``.  The value weights occupy the last
    third of the output dimension (rows ``2*d : 3*d``).  ViT-5-Base has
    ``qkv_bias=False``, so no bias term is present.

    Args:
        model: A ``vit_models`` (HookedViT5) instance.
